...
ORF_fluxchange_map <- ORF_fluxchange_map%>%
                      group_by(metal,ORF)%>%
                      mutate(flux_change_detected = any(flux_change_detected))%>%
                      ungroup()%>%
                      unique()
...
convert_numeles2colour <- function(x){
  viridis(7,option = "C",end = 0.9 ,direction = -1)[x]
}
...
df <- data.frame(num_eles_hit_in = 1:7, 
                 color = sapply(1:7, convert_numeles2colour))
