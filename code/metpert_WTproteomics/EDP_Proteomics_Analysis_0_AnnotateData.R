...
source(paste0(projdir,"/Common Functions/Layout_Conversions.R"))
source(paste0(projdir,"/Common Functions/Graphic_Parameters.R"))
source(paste0(proteomics_dir,'/EDP_Analysis_Functions.R'))
...
protODdir=paste0(proteomics_dir,"/ODs at harvest")
...
DIANNout_stats<-read.delim("EDP_24hr_DIANNout.stats.tsv",stringsAsFactors = F)%>%
...
dir.create(paste0(QCplotdir,"/AllDataStats"))
setwd(paste0(QCplotdir,"/AllDataStats"))
...
layout <- read.csv(paste0(projdir,"/CommonFiles/All_Layouts_Combined.csv"),stringsAsFactors = F)
...
for(i in 1:length(ODfiles)){
  ...
  odf=read_excel(ODfiles[i])[24:31,]
...
Spec2Tecan=read.csv(paste0("/camp/home/aulakhs/working/Simran Aulakh/Ionome Project/Element Depletion/EleDepChel January 2020/Metallomics/OD final/Spec2NE3tecan.csv"),stringsAsFactors = F) # Read in Spectrophotoeter to Tecan conversion file
...
Sdata<-as.data.frame(diann_load("EDP_24hr_DIANNout.tsv"))
...
Sdata <- merge(Sdata,OD_df[,c(4,5)],by="Plate_Position_TimePoint")

## Merge with layout

Sdata$Sample.Name = layout[match(Sdata$Plate_Position,layout$Plate_Position),"Condition.or.Strain"]
...
files_after_ODfilter <- unique(Sdata$File.Name)
...
AcSeq <- read.delim("acquistion_sequence.txt",sep="\t",stringsAsFactors = F)
...
Sdata<-merge(Sdata,AcSeq,by="File.Name")
...
write.csv(Sdata, "AllData_prepared_noPrecFilter.csv")
...
write.csv(ExpDesign[,c("label","condition","replicate")],
          "EDP_ExperimentDesign.csv",row.names = F)
write.csv(ExpDesign_AE[,c("label","condition","replicate")],
          "EDP_ExperimentDesign_AEonly.csv",row.names = F)
