...
# Need to run EDP_Proteomics_Analysis_ 0 and 1A before running this  
...
lm_DEres_plotdir =paste0(proteomics_dir,"/Plots/DE with Linear Models")
dir.create(lm_DEres_plotdir,recursive = T)

lm_DEres_elewiseplotdir=paste0(lm_DEres_plotdir,"/Elewise")
dir.create(lm_DEres_elewiseplotdir,recursive = T)
...
MeasConcAxis <- read.csv("/camp/lab/ralserm/working/Simran Aulakh/Ionome Project/Element Depletion/EleDepChel January 2020/Metallomics/Media Measurements/Measured_Element_Concentration_Axis_wBioSpecID.csv",stringsAsFactors = F)%>%
...
AllProtData <- read.csv("Protein Quantities w NAs.csv",
                        stringsAsFactors = F)%>%
...
setwd(lm_DEres_plotdir)
pdf("Num proteins by least complex model.pdf",width=6,height=6)
ggplot(lm_res_df_ms,
       aes(x=Element,
...
publish_gostplot(go_enrichres_plot, highlight_terms = go_enrich_res_df$term_id, 
                 width = 10, height = 10, 
                 filename = paste0(lm_DEres_plotdir,"/gprofiler_enrich_result_DE in any lm genes.pdf") )
...
publish_gostplot(go_enrichres_plot, highlight_terms = go_enrich_res_df$term_id, 
                 width = 10, height = 30, 
                 filename = paste0(lm_DEres_plotdir,"/gprofiler_enrich_result_quadratic genes.pdf") )
...
setwd(FinalOutCSVdir)
write.csv(lm_res_df,paste0("lm fit res pvthresh ",adjpvthresh,".csv"),row.names=F)
...
PQ_lm_res_df <- merge(AllProtData[,c("Genes","Element","BioSpecID",
  ...
  mutate(ORF = as.character(lapply(Genes,convert_GeneName2SingleORF)))

write.csv(PQ_lm_res_df,paste0("lmfit DE res with PQ and SigNotSig AdjPVthresh ",
                              adjpvthresh," FCthresh ",fcthresh,".csv"))
write.csv(PQ_lm_res_df,paste0(projdir,"/AllGenomeWideDatasets/lmfit DE res with PQ and SigNotSig AdjPVthresh ",
                              adjpvthresh," FCthresh ",fcthresh,".csv"))
...
pdf("Top 3 most significant linear.pdf",width=10,height=4)
...
pdf("Top 3 most significant quadratic.pdf",width=10,height=4)
...
pdf("Top 3 most significant cubic.pdf",width=10,height=4)
...
plot_allsig_permodel_perele <- function(df,modeltype,ele){
  ...
  if(nrow(df)>1){
    ...
    if(modeltype=="linear"){
      ...
      pdf(paste0("AllSig ",modeltype," fits along",ele," concgrad.pdf"),width = round(sqrt(nsiggen),0)*2.5,
      ...
      pdf(paste0("Top20 ",modeltype," fits along",ele," concgrad.pdf"),width = 20,
          height = 20)
...
top30_alleles<- filter(PQ_lm_res_df,LeastComplexModel != "null")%>%
  ...
  unique()%>%
  top_n(-30,PValAdj_BH)
write.csv(top30_alleles,"Top 30 across all metals.csv")
...
pdf("Total Number of Hits per condition lmDEres.pdf",width=10,height=8)
...
# Coloured by number of significant conditions

pdf("Total Number of Hits per condition lmDEres colby FracSigIn.pdf",width=10,height=8)

ggplot( )+
  geom_bar(data= DElmfit_res_sum_numsigcat,
           aes(y=Total_Sig_per_EleFracSig,
               x= Element,
               fill = factor(Fraction_Conditions_Significant )),
           colour="white",
           stat="identity",position = "stack")+
  geom_bar(data= unique(DElmfit_res_sum[,c("Element","Total_Tested")]),
           aes(y=Total_Tested,
               x=Element),
           fill=NA,
           colour=viridis(1),
  ...
  geom_text(data= unique(DElmfit_res_sum[,c("Element","Total_Tested","Total_Significant")]),
            aes(y=Total_Significant+200,
                x=Element,
                label=paste0("Tested:\n",Total_Tested,"\n","Significant:\n",Total_Significant)))+
  labs(title=paste0("DE genes padj < ",adjpvthresh, "\n abs(FC max minus min PQ) > ",fcthresh),
       y="Number of Genes",
       fill = "Fraction of Elements Significant In")+
  theme_SKA()+
  theme(legend.position="bottom")
dev.off()


DElmfit_res_sum_unique_vs_nonunique <- PQ_lm_res_df%>%
  dplyr::select(Genes,Element,Significant,MaxMag_LogFC)%>%
  filter(Element != "AllEle")%>%
  unique()%>%
  group_by(Genes)%>%
  mutate(Num_Conditions_Significant = sum(Significant),
         Unique = ifelse(Num_Conditions_Significant ==1 ,"Unique Hit","NonUnique Hit" )) %>%
  ungroup()%>%
  filter(Significant == 1)%>%
  group_by(Element,Unique)%>%
  summarize(Total_Sig_per_unq_nonunq= sum(Significant),
  )%>%
  ungroup()


# Coloured by number of significant conditions
pdf("Total Number of Hits per condition lmDEres colby unqnonunq.pdf",width=10,height=8)

ggplot( )+
  geom_bar(data= DElmfit_res_sum_unique_vs_nonunique,
           aes(y=Total_Sig_per_unq_nonunq,
               x= Element,
               fill = factor(Unique )),
           colour="white",
           stat="identity",position = "stack")+
...
plot_lmfitDEs<-function(df,ele){
  ...
  pdf(paste0("DE lm ",elename, i," degree lm summary.pdf"),width=8,height=8)
  print(vp)
  dev.off()
