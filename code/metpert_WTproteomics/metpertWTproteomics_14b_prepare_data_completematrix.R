...
diann_data_dir = paste0(metpert_WTproteomics_dir,"/DIANN_output")
plot_dir = paste0(metpert_WTproteomics_dir,"/output/plots/completematrix/allmetals")
QCplotdir = paste0(plot_dir,"/qc_plots")

dir.create(paste0(QCplotdir,"/AEcontrolsamples"),recursive = T)
dir.create(paste0(QCplotdir,"/allsamples"))


intermediate_datafiles_dir =paste0(paste0(metpert_WTproteomics_dir,"/output/intermediate_datafiles"))
output_tables_dir = paste0(metpert_WTproteomics_dir,"/output/tables/completematrix/allmetals")

dir.create(paste0(output_tables_dir,"/AllEleQC"), recursive = T)
dir.create(paste0(output_tables_dir,"/allsamples"), recursive = T)

###############################################################################
### Create precursor  filter based on CV of precursors in all ele controls ###
##############################################################################

AE_data<-read.csv(paste0(intermediate_datafiles_dir,"/AllData_prepared_noPrecFilter.csv"),stringsAsFactors = F)%>%
...
Sample_data <- read.csv(paste0(intermediate_datafiles_dir,"/AllData_prepared_noPrecFilter.csv"),stringsAsFactors = F)
...
Exp_Design<-read.csv(paste0(intermediate_datafiles_dir,"/EDP_ExperimentDesign_AEonly.csv"),stringsAsFactors = F)
...
pdf(paste0(QCplotdir,"/AEcontrolsamples/Effect of Filtering_AEmedCorrection_NAhandling FullMatrix on data.pdf"),
    width=20,height=20)
...
pdf(paste0(QCplotdir,"/AEcontrolsamples/Median protein Quantities across data processing FullMatrix stages.pdf"),
    width=40,height=20)
...
setwd(paste0(QCplotdir,"/AEcontrolsamples/"))
...
#write.csv(FinnoNA_df,paste0(output_tables_dir,"/AllEleQC/Protein Quantities wImpValues_AllEleOnly FullMatrix.csv"),row.names = F)
...
#write.csv(FinnoNA_df,paste0(output_tables_dir,"/AllEleQC/Protein Quantities wImpValues_AllEleOnly limmaBC FullMatrix.csv"),row.names = F)
...
pdf(paste0(QCplotdir,"/AEcontrolsamples/Median Protein Quantity after diann_maxlfq AEcorr medianscale FullMatrix.pdf"),width=20,height=10,family = "Times")
...
pdf(paste0(QCplotdir,"/AEcontrolsamples/Mean Protein Quantity after diann_maxlfq AEcorr medianscale FullMatrix.pdf"),width=20,height=10,family = "Times")
...
pdf(paste0(QCplotdir,("/AEcontrolsamples/Total Protein Quantity after diann_maxlfq AEcorr medianscale FullMatrix.pdf")),width=20,height=10,family = "Times")
...
Exp_Design<-read.csv(paste0(intermediate_datafiles_dir,"/EDP_ExperimentDesign.csv"),stringsAsFactors = F)

Exp_Design_AEonly<-read.csv(paste0(intermediate_datafiles_dir,"/EDP_ExperimentDesign_AEonly.csv"),stringsAsFactors = F)
...
pdf(paste0(QCplotdir,"/allsamples/Effect of Filtering_AEmedCorrection_NAhandling on data.pdf"),width=20,height=20)
...
pdf(paste0(QCplotdir,"/allsamples/Median protein Quantities across data processing stages.pdf"),width=40,height=20)
...
pdf(paste0(QCplotdir,"/allsamples/Protein Intensity profiles facet.pdf"),width=20,height=15)
for(i in 1:length(SEdfs)){
  plot_protint_dist_facet(SEdfs[[i]], names(SEdfs)[[i]])
  
}

pdf(paste0(QCplotdir,"/allsamples/Protein Intensity profiles allsamp.pdf"),width=20,height=15)
for(i in 1:length(SEdfs)){
...
#write.csv(Fin_wNA_df_limma,paste0(output_tables_dir,"/allsamples/Protein Quantities w NAs limmaBC FullMatrix.csv"),row.names = F)
...
#write.csv(AERep_Stats,paste0(output_tables_dir,"/allsamples/AllEle Replicates Protein Quantity Stats limmaBC FullMatrix.csv"),row.names=F)
...
#write.csv(AllSample_Stats,paste0(output_tables_dir,"/allsamples/AllSample Protein Quantity Stats limmaBC FullMatrix.csv"),row.names=F)
...
#write.csv(Elewise_Stats,paste0(allsamples,"/allsamples/Elewise Protein Quantity Stats limmaBC FullMatrix.csv"),row.names=F)
...
#write.csv(FinnoNA_df_limma,paste0(output_tables_dir,"allsamples/Protein Quantities wImpValues limmaBC FullMatrix.csv"),row.names = F)
...
Fin_wNA_df <-  merge(PG_quants_BCMS,sample_anno[,c(1,which(!colnames(sample_anno) %in% colnames(PG_quants_BCMS)))],by="File.Name")%>%
  ...
  mutate(BioSpecID=trimws(BioSpecID),
         `Log2(Protein.Quantity)` = log2(Protein.Quantity))%>%
  filter(!BioSpecID %in% c("AllEleprepQC 1","Mo 2","Mo 5"))


write.csv(Fin_wNA_df,paste0(output_tables_dir,"/allsamples/Protein Quantities w NAs FullMatrix.csv"),row.names = F)
...
write.csv(AERep_Stats,paste0(output_tables_dir,"/allsamples/AllEle Replicates Protein Quantity Stats FullMatrix.csv"),row.names=F)
...
write.csv(AllSample_Stats, paste0(output_tables_dir,"/allsamples/AllSample Protein Quantity Stats FullMatrix.csv"),row.names=F)
...
write.csv(Elewise_Stats,paste0(output_tables_dir,"/allsamples/Elewise Protein Quantity Stats FullMatrix.csv"),row.names=F)
...
write.csv(Ele_RepStats,paste0(output_tables_dir,"/allsamples/Protein Quantity Replicate Stats Branch C - completematrix allmetals.csv"),row.names=F)
...
write.csv(Ele_RepStats_Summary,paste0(output_tables_dir,
                                      "/allsamples/Protein Quantity Replicate Stats Summary Branch C-completematrix allmetals.csv"),row.names=F)
...
pdf(paste0(QCplotdir,"/allsamples/Number of proteins per Layout Branch C - completematrix.pdf"),width = 5,height=4)
ggplot(numprot,
...
pdf(paste0(QCplotdir,"/allsamples/Num protein per condition Dataset C - completematrix ylim.pdf"),width = 13,height=5)
ggplot(numprot,
