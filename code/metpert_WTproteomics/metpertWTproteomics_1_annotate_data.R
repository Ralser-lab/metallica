...
setwd(metpert_WTproteomics_dir)
...
write.csv(ExpDesign[,c("label","condition","replicate")],
          paste0(intm_data_dir,"/metpertWTproteomics_ExperimentDesign.csv"),row.names = F)

write.csv(ExpDesign_AE[,c("label","condition","replicate")],
          paste0(intm_data_dir,"/metpertWTproteomics_ExperimentDesign_AEonly.csv"),row.names = F)
