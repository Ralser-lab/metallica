...
col <- rev(colorRampPalette(colors = c("#67001F", "#B2182B", "#D6604D", "#F4A582", "#FDDBC7", "#F7F7F7", "#D1E5F0", "#92C5DE", "#4393C3", "#2166AC", "#053061"))(200))
...
pdf(paste0(plot_dir,"/Fe depletion dataset comparison.pdf"),width = 7.3,height = 5)
...
ggplot(Zn_depletion_all_ds_res,
  ...
  geom_vline(xintercept = log2(1.5), size = 0.2, colour = "darkred")+
  geom_vline(xintercept = log2(1/1.5), size = 0.2, colour = "darkblue")+
  geom_text_repel(aes(label = Gene),size = 4,max.overlaps = 50)+
  scale_colour_manual(values = c("darkblue","gray","darkred"))+
